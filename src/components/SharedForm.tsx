...
export default function SharedForm({ formId, buttonText = "Submit", buttonIcon, onSuccess }: SharedFormProps) {
  ...
  const getInputBorderClass = (isValid: boolean, isTouched: boolean) => {
    if (!isTouched) return "border-white/10 focus:border-cyan-400 focus:bg-white/10";
    return isValid 
  ...
  const labelClass = "absolute left-4 top-3.5 text-stone-400 text-sm transition-all peer-focus:-top-2.5 peer-focus:left-3 peer-focus:text-[10px] peer-focus:px-2 peer-focus:bg-[#050811]/90 peer-focus:backdrop-blur-md peer-focus:rounded peer-focus:text-cyan-400 peer-not-placeholder-shown:-top-2.5 peer-not-placeholder-shown:left-3 peer-not-placeholder-shown:text-[10px] peer-not-placeholder-shown:px-2 peer-not-placeholder-shown:bg-[#050811]/90 peer-not-placeholder-shown:backdrop-blur-md peer-not-placeholder-shown:rounded peer-not-placeholder-shown:text-cyan-400 pointer-events-none z-20";
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full" noValidate>
        ...
        <div className="flex gap-2">
          <div className="relative shrink-0 w-32">
            <select
              ...
              className="w-full h-full bg-white/5 backdrop-blur-md border border-white/10 rounded-xl pl-2.5 pr-7 py-3.5 text-white text-xs font-medium focus:outline-none focus:border-cyan-400 transition-all appearance-none cursor-pointer"
            >
              {COUNTRY_CODES.map((c) => (
                <option key={c.code + c.country} value={c.code} className="bg-[#050811] text-white">
                  {c.label}
                </option>
              ))}
            </select>
      ...
      <div className="relative mt-2 group">
        <div className="absolute -inset-1 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl blur opacity-0 group-hover:opacity-50 transition duration-1000 group-hover:duration-300 pointer-events-none"></div>
        <button 
          disabled={submitStatus !== 'idle'} 
          className={`relative w-full rounded-xl py-4 font-bold tracking-widest uppercase text-sm transition-all duration-500 flex items-center justify-center gap-2 border overflow-hidden ${submitStatus !== 'idle' ? 'text-white border-transparent' : 'bg-[#050811] border-cyan-500/40 text-cyan-50 hover:border-cyan-400 hover:shadow-[0_0_40px_rgba(34,211,238,0.35)] active:scale-[0.98]'}`}
        >
          {submitStatus === 'idle' && <div className="absolute inset-0 -translate-x-[150%] bg-gradient-to-r from-transparent via-cyan-200/15 to-transparent group-hover:animate-shimmer pointer-events-none"></div>}
          
          {submitStatus === 'loading' && <div className="absolute inset-0 bg-cyan-600 rounded-xl"></div>}
